...
USER = #lastfm username of person who you want to get songs listened to from
  ...
  while page_num > 0:
    ...
    try: # This stops it from breaking because of no recenttracks key
      ...
      for t in r.json()['recenttracks']['track']:
          if not "@attr" in t:
              tracks.append(
              [[f"{t['name']} by {t['artist']['#text']}",[t['date']['#text'].split()],[t['image'][3]['#text']]]]) # the number between ['image] and ['text] represents size
